...
* 캔버스에 **“Streamlit VOC Topic Explorer (Hierarchical + Alerts)”** 확장판을 올렸습니다.
* 추가된 기능:

  1. **계층형 토픽**: 클러스터 센트로이드를 KMeans로 묶어 **상위 슈퍼클러스터** 자동 생성 → Sunburst로 탐색
  2. **신규/급증 알림**: 주간 기준선 대비 **z-score 급증** 및 **완전 신규 토픽** 자동 감지 → 알림 테이블/다운로드
* 기존처럼 원본 JSON/NDJSON 또는 `docs.jsonl + summary.json` 업로드로 바로 사용 가능합니다.

---

### 어떻게 쓰면 되나요?

1. 좌측 코드 파일을 `app.py`로 저장하고 의존성 설치:

   ```bash
   pip install -U streamlit pandas numpy scikit-learn sentence-transformers hdbscan umap-learn plotly
   # (선택) BERTopic 엔진까지 쓰려면:
...
3. 사이드바에서 모드 선택:
   ...
   * **원본 JSON에서 생성**: JSON/NDJSON 업로드 → 텍스트 필드 선택 → 실행
   * **미리 계산된 결과 시각화**: `voc_topics_docs.jsonl` + `voc_topics_summary.json` 업로드
...
* **계층형 토픽(슈퍼클러스터)**
  ...
  * “설정 및 생성”에서 `K` 슬라이더로 상위 토픽 개수 조절 → **Sunburst**로 `Super → Cluster` 구조 탐색
  * 슈퍼라벨은 c-TF-IDF 키워드 기반으로 자동 생성
  * **계층 매핑 CSV** 다운로드 버튼 제공

* **UMAP 산점도**

  * 색상 기준을 **cluster** 또는 **super_id** 중 선택 가능

* **신규/급증 알림**

  * 날짜 칼럼명을 입력(예: `created_at`) → 윈도우(주), z-score 임계치, 최소건수 설정
  * **급증(spike)**: 최근 주가 기준선보다 z ≥ 임계 & 건수 ≥ 최소건수
  * **신규(new)**: 기준선 기간 동안 0건 & 최근 주 최소건수 이상
  * 알림 표/라인차트 표시 + **CSV 다운로드** 제공

---

필요하시면 아래도 추가해 드릴 수 있어요:

* **계층형 라벨 가이드라인(사전/금칙어 반영)**
* **자동 한글 라벨 최적화(15자 이내 요약 라벨러)**
* **변화 감지 고도화**: 주중 계절성 보정, CUSUM/단변량 베이지안 체인지포인트, 주간 대비 전년 동기 비교 등.
