...
export async function fetchSource(source: string) {
  const res = await fetch(`${API_BASE}/source?source=${encodeURIComponent(source)}`)
  if (!res.ok) throw new Error(await res.text())
  return await res.json() as { source: string; text: string }
}

export async function compactOne(source: string, text?: string) {
  const res = await fetch(`${API_BASE}/admin/compact_one`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function profileStream(prompt: string, models: string[], mode: 'basic'|'hybrid'|'none' = 'basic', k: number = 4) {
  const res = await fetch(`${API_BASE}/profile/stream`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ prompt, models, mode, k })
  })
  if (!res.ok) throw new Error(await res.text())
  return await res.json() as { prompt: string; mode: string; results: {model:string; first_token_ms:number; total_ms:number; tokens:number; tps:number}[] }
}
