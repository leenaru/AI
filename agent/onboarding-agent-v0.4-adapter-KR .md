...
이 문서는 **모델 어댑터 레이어**가 포함된 서버 소스 전체와 사용법을 한 파일로 제공합니다.

## 핵심 포인트
- 백엔드(vLLM/Ollama/OpenAI/TGI) 교체 시 **그래프/서비스 로직 수정 없음**
- `llm_adapter.chat(model, messages)` 인터페이스만 사용
- 모델명 선택은 `model_router.py`, 백엔드 선택은 `.env`의 `LLM_BACKEND`

## 빠른 시작
```bash
cd server
cp .env.example .env
# LLM_BACKEND=vllm | ollama | openai | tgi 로 선택
docker compose up -d
...
- `server/app/services/adapters/base.py`
...
### README.md
```md
# Onboarding Agent – v0.4 (Model Adapter Layer)

본 버전은 **Model Adapter Layer**를 도입하여, 백엔드(vLLM / Ollama / OpenAI 호환 / HF TGI)가 바뀌어도
LangGraph 및 서비스 로직 수정 없이 동작합니다.

- `app/services/adapters/` : 어댑터 인터페이스 및 구현체
- `app/services/model_router.py` : 언어/정책 기반 모델명 선택
- `app/services/adapters/registry.py` : 환경변수 기반 어댑터 선택/생성
- LangGraph 노드에서는 **항상 동일 인터페이스**(`llm_adapter.chat(...)`)만 사용합니다.
...
# ==== Adapter/Backend switch ====
LLM_BACKEND=vllm           # vllm | ollama | openai | tgi
...
async def get_ticket(tid: str):
    if tid not in TICKETS: raise HTTPException(404, "not found")
    return TICKETS[tid]

@router.patch("/tickets/{tid}/resolve")
async def resolve_ticket(tid: str, body: TicketResolve):
    if tid not in TICKETS: raise HTTPException(404, "not found")
    t = TICKETS[tid]; t.status = "resolved"; t.notes.append(body.resolution)
...
setup_logging("INFO")
app = FastAPI(title="On-device + Server AI – API", version="0.4.0")
...
class OllamaAdapter:
    ...
    async def chat(self, model: str, messages: List[Dict[str, Any]], **kwargs) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=60) as cx:
            r = await cx.post(f"{self.base}/api/chat", json=payload)
            r.raise_for_status()
            data = r.json()
...
def get_adapter():
    name = (settings.llm_backend or "vllm").lower()
    if name == "ollama":
...
from app.services.adapters.registry import llm_adapter
...
async def intent_router_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    elif any(k in text for k in ["고장", "에러", "오류", "안됨", "소움", "누수"]): state["route"] = "troubleshoot_flow"
...
async def require_product_context(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not prod.get("model"):
        state.setdefault("actions", []).append({"type": "client_action","action": "REQUEST_INFO","params": {"fields": ["brand","model","serial"]},"safety": {"requires_user_confirm": False}})
        state["answer"] = "제품 모델명/시리얼을 알려주세요. 필요 시 제품 라벨 촬영을 도와드리겠습니다."
    return state
...
간결히 요약 답변."}]
    llm = await llm_adapter.chat(model=model, messages=messages)
...
LANG_MODEL_MAP_OLLAMA = {"ko": "llama3:instruct", "en": "llama3:instruct"}
...
def choose_model(lang: str | None, policy_country: Optional[str]) -> str:
    ...
    if backend == "tgi":
        return settings.tgi_model
    # ollama
    if lang:
        key = lang.split("-")[0]
        if key in LANG_MODEL_MAP_OLLAMA:
            return LANG_MODEL_MAP_OLLAMA[key]
...
class PolicyEnforcer:
    def __init__(self, rules: Dict | None = None): self.rules = rules or {}
    def require_citations(self) -> bool: return bool(self.rules.get("require_citations", True))
    def mask_pii(self, text: str) -> str:
        masked = text
        for t in SENSITIVE_TOKENS: masked = masked.replace(t, "[민감정보]")
        return masked
    def filter_topics(self, text: str) -> str:
        prohibited = self.rules.get("prohibited_topics", [])
        for topic in prohibited:
            if topic in text: return "요청하신 내용은 정책상 제공할 수 없습니다."
        return text
    def enforce(self, answer: str, citations: List[Dict]) -> tuple[str, List[Dict]]:
        ...
        if self.require_citations() and not citations: out += "
...
def embed(texts: list[str], model_name: str) -> list[list[float]]:
    ...
    return em.encode(texts, normalize_embeddings=True).tolist()
...
class QdrantIndexer:
    ...
    def search(self, query: str, limit: int = 5, filters: dict | None = None):
        ...
        if filters:
            conds = []
            if filters.get("tags"): conds.append(FieldCondition(key="tags", match=MatchAny(any=filters["tags"])))
            if filters.get("products"): conds.append(FieldCondition(key="products", match=MatchAny(any=filters["products"])))
            if conds: f = Filter(must=conds)
        res = self.client.search(collection_name=self.collection, query_vector=qv, limit=limit, query_filter=f)
        out = []
        for r in res:
            p = r.payload or {}
            out.append({"id": r.id, "score": r.score, "title": p.get("title"), "url": p.get("url"),
                        "date": p.get("date"), "tags": p.get("tags", []), "products": p.get("products", []),
                        "content": ""})
        return out
...
```
### server/docker-compose.yml
> **설명:** vLLM, Ollama, Qdrant 서비스를 함께 정의(옵션 사용 가능).
