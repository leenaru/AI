...
class ModelRouter:
  def __init__(self, providers: Dict[str, object], prefer: list):
    self.providers, self.prefer = providers, prefer
  @classmethod
  def load_from_yaml(cls, path: str):
    with open(path,"r",encoding="utf-8") as f: cfg=yaml.safe_load(f)
    providers={"ollama":OllamaProvider(model=cfg["ollama"]["model"], base_url=cfg["ollama"]["base_url"]),
  ...
  def generate(self, prompt: str)->str:
    for key in self.prefer:
      ...
      if prv.health():
        try: return prv.generate(prompt)
