...
PARQUET_DIR, FAISS_DIR = "data/parquet","data/faiss"
_emb = SentenceTransformer("intfloat/multilingual-e5-small")
_ce = None
cfg = {"k_dense":40,"k_sparse":40,"k_graph":2,
       "weights":{"alpha":0.55,"beta":0.25,"gamma":0.15,"delta":0.05},
       "cross_encoder":{"enabled":True,"top_m":10,"model":"BAAI/bge-reranker-v2-m3"}}
def _load_units(): return pd.read_parquet(os.path.join(PARQUET_DIR,"units.parquet"))
def _faiss_index(): return faiss.read_index(os.path.join(FAISS_DIR,"hnsw.index"))
_units_cache, _bm25 = None, None
def hybrid_search(q: str) -> Dict[str, Any]:
  global _units_cache, _bm25, _ce
  if _units_cache is None:
    _units_cache=_load_units(); _bm25 = BM25(_units_cache["summary"].tolist())
  qv=_emb.encode(["query: "+q], normalize_embeddings=True).astype("float32")
  D,I=_faiss_index().search(qv, cfg["k_dense"]); dense_scores = 1 - (D[0])
  idx_bm25, bm25_scores = _bm25.search(q, k=cfg["k_sparse"]) if cfg["k_sparse"]>0 else ([],[])
  now=int(time.time()); recency=(now - _units_cache.loc[I[0],"created_at"]).clip(lower=1)
  recency_scores = 1/np.log(recency+2.71828)
  candidates={}
  def add(idx, s_dense=0, s_bm25=0, s_graph=0, s_rec=0):
    if idx not in candidates: candidates[idx]={"dense":0,"bm25":0,"graph":0,"recency":0}
  ...
  for rank, idx in enumerate(I[0]): add(int(idx), s_dense=float(dense_scores[rank]), s_rec=float(recency_scores[rank]))
  for rank, idx in enumerate(idx_bm25): add(int(idx), s_bm25=float(bm25_scores[rank]))
  w=cfg["weights"]
  ...
  if cfg["cross_encoder"]["enabled"]:
    if _ce is None: _ce = CrossEncoder(cfg["cross_encoder"]["model"])
    ces = _ce.predict([(q, text) for _,text in rows])
    for idx,(i,_) in enumerate(rows): candidates[i]["score"]=float(ces[idx])
