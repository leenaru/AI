...
async def run_conversation(payload: Dict[str, Any]):
    ...
    with trace("conversation"):
        intent = await detect_intent(state); state["intent"] = intent
        plan = await plan_steps(state); state["plan"] = plan
        exec_out = await execute_tools(state); state.update(exec_out)
        draft = await synthesize(state); state["draft"] = draft
        final = await translate(state); state["final"] = final
