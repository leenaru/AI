# Onboarding Agent — vLLM Switchable Monorepo (v0.3)
**What’s inside**
- `server/`: FastAPI + LangGraph + **LLM backend switch (vLLM ⟷ Ollama)**, HQ‑RAG (in‑memory / optional Qdrant), appliance flows, tickets & RAG ingest.
- This file is a single, portable README that contains **all source files inline** below.

**Backend switch**
- `.env` → `LLM_BACKEND=vllm | ollama`
- vLLM: OpenAI‑compatible server (`vllm/vllm-openai`), configured via `VLLM_MODEL`, `VLLM_BASE_URL`.
- Ollama: local dev/edge; `DEFAULT_MODEL`, `OLLAMA_BASE_URL`.
...
cp .env.example .env
# choose backend
# LLM_BACKEND=vllm            # and set VLLM_MODEL=mistralai/Mistral-7B-Instruct-v0.3
# or LLM_BACKEND=ollama
docker compose up -d

# (optional) for Ollama dev:
